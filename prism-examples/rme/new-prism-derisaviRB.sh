...
for combo in "${combinations[@]}"; do
	...
	for i in $(seq 1 $num_iterations); do
		(time -p ./prism -javamaxmem 5000m -DerisaviRedBlack -explicit  -explicitbuild -importtrans /tmp/rme.tra -importlabels /tmp/rme.lab -pf "$propertie" -dtmc) 2>&1 | awk '/^user/ {printf ","$2}' >> "/tmp/rme/$DIR/time_DerisaviRedBlack.csv"
	done
	echo "" >> "/tmp/rme/$DIR/time_DerisaviRedBlack.csv"	

	./prism -javamaxmem 5000m -DerisaviRedBlack -explicit  -explicitbuild -importtrans /tmp/rme.tra -importlabels /tmp/rme.lab -pf "$propertie" -dtmc | grep "Minimisation" # test
