...
for combo in "${combinations[@]}"; do
	...
	for i in $(seq 1 $num_iterations); do
		(time -p ./prism -javamaxmem 3000m -explicit  -explicitbuild -importtrans /tmp/rme.tra -importlabels /tmp/rme.lab -pf "$propertie" -dtmc) 2>&1 | awk '/^user/ {printf ","$2}' >> "/tmp/rme/$DIR/time_Without.csv"
	done
	echo "" >> "/tmp/rme/$DIR/time_Without.csv"	

	./prism -javamaxmem 3000m -explicit  -explicitbuild -importtrans /tmp/rme.tra -importlabels /tmp/rme.lab -pf "$propertie" -dtmc | grep "Result" # test
