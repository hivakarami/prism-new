 ...
 * a text file containing its uniformization as a DTMC in the format given by PRISM/MRMC.
 * 
 * @author Maeve Wildes
	...
	public static void main (String [] args) {
		...
		if (args.length != ARGS) {
			...
			try {
				...
				// compute the total and maximal exit rates
				final double[] totalRate = new double[states + 1];
				for (int i = 0; i < transitions; i++) {
				    totalRate[source[i]] += rate[i];
				}

				// compute max total rate
				double maxTotalRate = 0.0;
				for (int i = 0; i < states; i++) {
				    maxTotalRate = Math.max(maxTotalRate, totalRate[i]);
				}

				// compute probabilities from each state to itself
				String loops = "";
				for (int i = 0; i < states; i++) {
				    if (totalRate[i] < maxTotalRate) {
					loops += ("%d %d %." + (precision - 1) + "f%n", i, i, 1 - totalRate[i]);
					transitions++;
				    }
				}
