...
for combo in "${combinations[@]}"; do
	...
	./prism -javamaxmem 3000m -explicit -importtrans /tmp/cdp.tra -importlabels /tmp/cdp.lab -pf "$propertie" -dtmc | grep "Result" # test
	...
	for i in $(seq 1 $num_iterations); do
		(time -p ./prism -javamaxmem 3000m -explicit -importtrans /tmp/cdp.tra -importlabels /tmp/cdp.lab -pf "$propertie" -dtmc) 2>&1 | awk '/^user/ {printf ","$2}' >> "/tmp/cdp/$DIR/time_Standard.csv"
	done
