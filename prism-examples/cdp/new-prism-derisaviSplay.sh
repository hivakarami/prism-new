...
for combo in "${combinations[@]}"; do
	...
	for i in $(seq 1 $num_iterations); do
		(time -p ./prism -javamaxmem 3000m -DerisaviSplayTree -explicit -importtrans /tmp/cdp.tra -importlabels /tmp/cdp.lab -pf "$propertie" -dtmc) 2>&1 | awk '/^user/ {printf ","$2}' >> "/tmp/cdp/$DIR/time_DerisaviSplayTree.csv"
	done
	echo "" >> "/tmp/cdp/$DIR/time_DerisaviSplayTree.csv"	

	./prism -javamaxmem 3000m -DerisaviSplayTree -explicit -importtrans /tmp/cdp.tra -importlabels /tmp/cdp.lab -pf "$propertie" -dtmc | grep "Minimisation" # test
