...
for combo in "${combinations[@]}"; do
	...
	for i in $(seq 1 $num_iterations); do
		(time -p ./prism -algo explicit.Valmari -explicitbuild -importtrans /tmp/cdp.tra -importlabels /tmp/cdp.lab -pf "$propertie" -dtmc) 2>&1 | awk '/^user/ {printf ","$2}' >> "/tmp/cdp/$DIR/time_Valmari.csv"
	done
	echo "" >> "/tmp/cdp/$DIR/time_Valmari.csv"	

	./prism -algo explicit.Valmari -explicitbuild -importtrans /tmp/cdp.tra -importlabels /tmp/cdp.lab -pf "$propertie" -dtmc | grep "Minimisation:" # test
	...
	for i in $(seq 1 $num_iterations); do
		(time -p ./prism -algo explicit.DerisaviSplayTree -explicitbuild -importtrans /tmp/cdp.tra -importlabels /tmp/cdp.lab -pf "$propertie" -dtmc) 2>&1 | awk '/^user/ {printf ","$2}' >> "/tmp/cdp/$DIR/time_DerisaviSplayTree.csv"
	done
	echo "" >> "/tmp/cdp/$DIR/time_DerisaviSplayTree.csv"	

	./prism -algo explicit.DerisaviSplayTree -explicitbuild -importtrans /tmp/cdp.tra -importlabels /tmp/cdp.lab -pf "$propertie" -dtmc | grep "Minimisation:" # test
	...
	for i in $(seq 1 $num_iterations); do
		(time -p ./prism -algo explicit.DerisaviRedBlack -explicitbuild -importtrans /tmp/cdp.tra -importlabels /tmp/cdp.lab -pf "$propertie" -dtmc) 2>&1 | awk '/^user/ {printf ","$2}' >> "/tmp/cdp/$DIR/time_DerisaviRedBlack.csv"
	done
	echo "" >> "/tmp/cdp/$DIR/time_DerisaviRedBlack.csv"	

	./prism -algo explicit.DerisaviRedBlack -explicitbuild -importtrans /tmp/cdp.tra -importlabels /tmp/cdp.lab -pf "$propertie" -dtmc | grep "Minimisation:" # test
